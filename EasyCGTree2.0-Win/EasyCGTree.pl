...
print LOG "\n\n\n############### Step 2: Blast Search ###############\n\n";
	...
	if ($mode eq "A3")	{
		foreach my $file (@queryR) {
			...
			my $cmd;
			if ($tblastn) {
				$cmd="$tblastn"."$blast -query $query -db ./$TEMdir/TEM1_blastDB/$file -out ./$TEMdir/TEM2_blastOUT/Reference_$file -evalue 1e-5 -num_threads $num_threads".' -outfmt "6 qseqid sseqid sstart send pident qcovs evalue bitscore"';
			} else {
				$cmd="$blast -query $query -db ./$TEMdir/TEM1_blastDB/$file -out ./$TEMdir/TEM2_blastOUT/Reference_$file -evalue 1e-5 -num_threads $num_threads".' -outfmt "6 qseqid sseqid sstart send pident qcovs evalue bitscore"';
			}
			$number++;
...
foreach my $in5 (@in1) {
	...
	next unless $in5 ~~ @finalgenome || $ad0id ~~ @finalgenome;	
...
my $cmd11;
if ($seqtype eq "nucl") {
	$cmd11=".\\bin\\FastTree.exe -gtr -nt $inputDir.concatenation.fas > $inputDir.tree";##########################%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
} elsif ($seqtype eq "prot") {
	$cmd11=".\\bin\\FastTree.exe $inputDir.concatenation.fas > $inputDir.tree";	##########################%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
}
