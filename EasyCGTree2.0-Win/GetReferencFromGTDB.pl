...
Usage:\n perl_script + List.txt SeqDIR ID
...
foreach my $file (@query) {
	...
	$rrr++;
	print "Readling $inputDir/$file ($rrr of 120/122 files)...\n";#####
...
foreach my $file (@query) {
	...
	foreach my $i (@list) {
		...
		$lack{$i} .="$file " if $lack{$i};###
		$lack{$i} ="$file " unless $lack{$i};####
		$lnum{$i}++ if $lnum{$i};
...
print OU "$head\n";
my $cmd1="md $ARGV[2]";
system $cmd1 || die "Can't create blastDB directory '$ARGV[2]'\n";
