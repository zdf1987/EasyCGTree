...
-blast_dir <String> (Optional)
	Directory for programs of blast+. Please use double backslash to escape. [default: .\\bin\\]
...
foreach (@finalgenelist) {
	my $program = ".\\bin\\muscle.exe";##Different from Linux version
	my $outname = $_ . ".fas.fasta";
	my @para = ("-in", "./$TEMdir/TEM6_DedupGeneCluster/$_.fas", "-out", "./$TEMdir/TEM7_Alignment/$outname");##Different from Linux version
	unless (system "$program", @para) {##Different from Linux version
		$num7++;
...
if ($tree_type eq "sm") {
	...
	my $cmd11;
	if ($seqtype eq "nucl") {
		$cmd11=".\\bin\\FastTree.exe -gtr -nt $inputDir.concatenation.fas > $inputDir.supermatrix.tree";##Different from Linux version
	} elsif ($seqtype eq "prot") {
		$cmd11=".\\bin\\FastTree.exe $inputDir.concatenation.fas > $inputDir.supermatrix.tree";	##Different from Linux version
	}
	...
	foreach my $file11x ( @finalgenelist) {
		...
		my $cmd11;
		if ($seqtype eq "nucl") {
			$cmd11=".\\bin\\FastTree.exe -gtr -nt $TEMdir/TEM9_ReCallAln/$file11 > $TEMdir/TEM11_GeneTrees/$file11.tree";##Different from Linux version
		} elsif ($seqtype eq "prot") {
			$cmd11=".\\bin\\FastTree.exe $TEMdir/TEM9_ReCallAln/$file11 > $TEMdir/TEM11_GeneTrees/$file11.tree";	##Different from Linux version
		}
		&FastreeResponse ($cmd11);	
	}
	...
	close OU11;
	my $cmd11t;
	$cmd11t=".\\bin\\consenseM.exe ";##Different from Linux version
	...
	my $cmd11=".\\bin\\FastTree.exe -gtr -nt $inputDir.concatenationSNP.fas > $inputDir.SNP.tree";##Different from Linux version
